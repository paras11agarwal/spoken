...
$(".fancybox").fancybox({maxWidth:500,maxHeight:485,fitToView:true,autoSize:false,closeClick:false,openEffect:"none",closeEffect:"none",modal:true,});$(".video-show-more").click(function(e){e.preventDefault();$(".video-info").slideToggle("slow");$(this).text($(this).text()=='Show video info'?"Hide video info":"Show video info");});});var player,tag=document.createElement('script'),firstScriptTag=document.getElementsByTagName('script')[0];var flag=false;var trackedPlayer=document.getElementById('video1');var currentTime=trackedPlayer.currentTime;var cur_time=formatTime(currentTime).toString();trigger(cur_time);tag.src="https://www.youtube.com/iframe_api";firstScriptTag.parentNode.insertBefore(tag,firstScriptTag);function onPlaybackQualityChange(event)
{console.log(event);}
...
{window.setTimeout(logDuration,1000);var cur_time=formatTime(ik_player.getCurrentTime()).toString();var duration=formatTime(ik_player.getDuration()).toString();if(parseFloat(cur_time)==parseFloat(duration.toString()))
{$('.ScrollStyle').find('.question').css("background-color","white");}
else
{trigger(cur_time,duration);}
return cur_time}
logDuration();$("#text").click(function(e)
...
{var el=document.getElementById('forums_Q');el.scrollTop=0;}
...
{div.style.backgroundColor="#ffffff";}}
var trackedPlayer=document.getElementById('video1');var currentTime=trackedPlayer.currentTime;var cur_time=formatTime(currentTime).toString();trigger(cur_time);$("#text").click(function(e)
{var trackedvideo=document.getElementById('video1');var currentTime=trackedvideo.currentTime;var cur_time=formatTime(currentTime).toString();trackaudio.pause();trackedPlayer.pause();setForum(cur_time);});function setForum(cur_time)
