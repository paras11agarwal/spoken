$(function()
{var medias={audio:Popcorn("#st_audio"),video:Popcorn("#st_video")},loadCount=0,events="play pause timeupdate seeking volumechange".split(/\s+/g);Popcorn.forEach(medias,function(media,type){media.on("canplayall",function(){this.emit("sync");}).on("sync",function(){if(++loadCount==2){events.forEach(function(event){medias.video.on(event,function(){if(event==="timeupdate"){if(!this.media.paused){return;}
medias.audio.emit("timeupdate");return;}
if(event==="seeking"){medias.audio.currentTime(this.currentTime());}
if(event==="volumechange"){if(this.muted()){medias.audio.mute();}else{medias.audio.unmute();}
medias.audio.volume(this.volume());}
if(event==="play"||event==="pause"){medias.audio[event]();}});});}});});});$(document).ready(function(){currentVideo=$('.playlist-item.active');nextVideo=currentVideo.next('li');prevVideo=currentVideo.prev('li');var tmpHtml=currentVideo.attr('data')+'/'+$('.video-count').html()
